...
var toggleActive = function()  {
    ...
    var processResult = function(
      ...
      //console.log("sf result='" + result + "', status='" + status + "', jqXHR='" + jqXHR + "', user_id='" + user_id + "'");
        $('#active-' + subscription_config_id).html(result);
   };

   $.ajax(
       {
          url: '/studioadmin/site-config/ajax-toggle-subscription-config-active/',
          data: {"subscription_config_id": subscription_config_id},
          type: 'POST',
          dataType: 'html',
...
const processDeleteSubscriptionConfig = function()  {
   ...
   const subscription_config_id = $button_just_clicked_on.data('subscription_config_id');
   ...
   const processResult = function(
      ...
      //console.log("sf result='" + result.attended + "', status='" + status + "', jqXHR='" + jqXHR + "', booking_id='" + booking_id + "'");
       ...
       if(result.deleted === true) {
           $('#row-' + subscription_config_id).hide();
       }
   ...
   $.ajax(
       {
          url: '/studioadmin/site-config/subscription-config/' + subscription_config_id + '/delete/' ,
          type: "POST",
          dataType: 'json',
          success: processResult,
...
$(document).ready(function()  {
  ...
  $('.subscription-config-delete-btn').click(_.debounce(processDeleteSubscriptionConfig, MILLS_TO_IGNORE, true));

   $(".add-subscription-config").click(function(ev) { // for each add url
        ev.preventDefault(); // prevent navigation
        var url = $(this).data("form"); // get the form url
        $("#AddSubscriptionConfigModal").load(url, function() { // load the url into the modal
